...
begin
  ...
  File.open('text.txt') do |f|
    f.each do |record|
      @lines += 1 # if this is an array, eventually, then we should be able to say @lines.size for this value
      @all_txt << record
      @lines_ary << record.lines # lines to array -- See above note on lines...

      #  puts "#{@lines} #{record}"

      # sum total characters
      @length = record.length # This gets reset each time.  Do we keep it somewhere?  Are we interested in what the length of the last record was?
      @characters += @length # @characters += record.length is as simple and expressive as it should be.

      # total words
      rec_words = record.split(' ')
      @words = rec_words.size # This is also inside an iterator, so gets reset each time.
      # @words if collected throughout the life if the iteration, can later be counted as @words.count one time, rather than each time.
      @word_total += @words

      # total characters exclude spaces
      rec_words.each do |elem|
        chars = elem.size
        @charactersexcludingspaces += chars # Everywhere else you seem to use name_separated_with_spaces... but not here...
      end
    end # end read loop
    ...
    @avg_words_sent = 0.0 # you initialize it to 0
    @avg_words_sent = @word_total / @sentence_tot # Then immediately throw away the value and re-write it.and you are doing Integer math... 
    @avg_sent_paragraph = 0.0 # Same thing here...
    @avg_sent_paragraph = @sentence_tot / @paragraphs.size

    # Averse to Heredoc?  How about??? Why does this work?
    puts [
      '------------------------------- Analysis Report -----------------------------------------------------',
      "Number of lines :  #{@lines}",
      "Number of characters  :  #{@characters}",
      "Number of characters excluding spaces   :  #{@charactersexcludingspaces}",
